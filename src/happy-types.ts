...
export type GetAttribute<T extends string> = <N extends Container | HTML>(
  node: N
) => string;

export type NodeTypeInput<T extends ReturnType<typeof getNodeType>> =
  T extends "html"
    ? string
    : T extends "text"
    ? IText
...
export type UpdateSignature =
  | [el: IElement, idx: number, total: number]
  | [el: IElement, idx: number]
  | [el: IElement];
 ...
 */
export type UpdateCallback_Native = (
  el: IElement,
  idx: number,
  total: number
) => IElement | false;
 ...
 */
export type NodeSolverReady<E extends NodeType, O> = <
  N extends Exclude<Container | HTML | null, E>
>(
  node: N,
  parent?: IElement | DocRoot
) => O extends "mirror" ? N : O;
 ...
 */
export type NodeSolver = <E extends NodeType = never>(
  ...exclude: E[]
) => NodeSolverWithExclusions<E>;
...
export interface NodeSelector<T extends Container | "html"> {
   ...
   */
  update: <E extends string | boolean>(
    sel?: string,
    errorMsg?: E
  ) => <CB extends UpdateCallback>(cb: CB) => NodeSelector<T>;
  /**
